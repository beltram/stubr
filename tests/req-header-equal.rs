...
#[test]
fn should_map_request_equal_header() {
...
#[test]
fn should_not_match_when_not_equal_header_key() {
    let server = mount("req/headers/equal/string");
    let response = block_on(surf::get(&server.uri())
...
#[test]
fn should_not_match_when_not_equal_header_value() {
    let server = mount("req/headers/equal/string");
    let response = block_on(surf::get(&server.uri())
        .set_header("Content-Type", "application/not-json")).unwrap();
    assert_eq!(response.status().as_u16(), 404);
}
