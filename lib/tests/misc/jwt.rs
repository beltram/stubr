...
mod payload {
    ...
    mod eq {
        ...
        async fn should_match_payload_by_equality() {
            get(stubr.uri())
                .header("Authorization", format!("Bearer {}", JDOE_TOKEN)).await
                .expect_status_ok();
        }
        ...
        async fn should_fail_when_payload_does_not_match() {
            get(stubr.uri())
                .header("Authorization", format!("Bearer {}", ADOE_TOKEN)).await
                .expect_status_not_found();
        }
        ...
        async fn should_fail_when_payload_empty() {
            get(stubr.uri())
                .header("Authorization", format!("Bearer {}", VOID_TOKEN)).await
                .expect_status_not_found();
        }
        ...
        async fn should_fail_when_missing_bearer_prefix() {
            get(stubr.uri())
                .header("Authorization", JDOE_TOKEN).await
                .expect_status_not_found();
        }
    ...
    mod json_path {
        ...
        async fn should_fail_when_json_path_does_not_match() {
            get(stubr.uri())
                .header("Authorization", format!("Bearer {}", FRENCH_TOKEN)).await
                .expect_status_not_found();
        }
