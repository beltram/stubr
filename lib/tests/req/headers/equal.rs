...
async fn should_map_request_exact_int_value() {
    ...
    get(&srv.uri()).header("Content-Type", "42").await.expect_status_ok();
}

#[async_std::test]
async fn should_fail_when_incorrect_int_value() {
    let srv = given("req/headers/equal/int");
    get(&srv.uri()).header("Content-Type", "43").await.expect_status_not_found();
}
...
async fn should_map_request_exact_bool_value() {
    ...
    get(&srv.uri()).header("Content-Type", "true").await.expect_status_ok();
}

#[async_std::test]
async fn should_fail_when_incorrect_bool_value() {
    let srv = given("req/headers/equal/bool");
    get(&srv.uri()).header("Content-Type", "false").await.expect_status_not_found();
}
