...
async fn should_map_request_when_header_matches() {
    ...
    get(&srv.url()).header("Content-Type", "application/xml").await.unwrap().assert_ok();
}
...
async fn should_fail_when_regex_not_respected() {
    let srv = given("req/headers/matches/single");
    get(&srv.url()).header("Content-Type", "text/plain").await.unwrap().assert_not_found();
    get(&srv.url()).header("Content-Type", "app/json").await.unwrap().assert_not_found();
}
...
async fn negative_should_map_request_when_header_matches() {
    ...
    get(&srv.url()).header("Content-Type", "application/json").await.unwrap().assert_ok();
    get(&srv.url()).header("Content-Type", "any").await.unwrap().assert_ok();
}
...
async fn negative_should_fail_when_one_of_header_regex_not_respected() {
    ...
    get(&srv.url())
        ...
        .assert_not_found();
    get(&srv.url()).header("Content-Type", "application/json").await.unwrap().assert_not_found();
    get(&srv.url()).header("X-Some", "application/json").await.unwrap().assert_not_found();
    get(&srv.url()).await.unwrap().assert_not_found();
}
