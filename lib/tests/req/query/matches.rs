...
async fn should_map_request_when_many_query_matches() {
    get(stubr.queries(("age", "string"), ("city", "string"))).await.expect_status_ok();
}

#[async_std::test]
#[stubr::mock("req/query/matches/many.json")]
async fn should_fail_when_one_of_query_regex_not_respected() {
    get(stubr.queries(("age", "1234"), ("city", "string"))).await.expect_status_not_found();
    get(stubr.queries(("age", "string"), ("city", "1234"))).await.expect_status_not_found();
    get(stubr.queries(("age", "1234"), ("city", "1234"))).await.expect_status_not_found();
...
async fn negative_should_fail_when_one_of_query_regex_not_respected() {
    get(stubr.queries(("age", "string"), ("city", "1234"))).await.expect_status_not_found();
    get(stubr.queries(("age", "1234"), ("city", "string"))).await.expect_status_not_found();
    get(stubr.queries(("age", "string"), ("city", "string"))).await.expect_status_not_found();
    get(stubr.query("age", "1234")).await.expect_status_not_found();
