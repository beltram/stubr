...
async fn should_fail_when_req_body_value_not_equal_to_json_string() {
    post(stubr.uri()).body(json!({"name": "not-bob"})).await.expect_status_not_found();
}
...
async fn should_fail_when_req_body_key_not_equal_to_json_bool() {
    post(stubr.uri()).body(json!({"isNotGood": true})).await.expect_status_not_found();
}
...
async fn should_map_req_body_equal_to_obj() {
    post(stubr.uri()).body(json!({"user": {"name": "jdoe"}})).await.expect_status_ok();
}

#[async_std::test]
#[stubr::mock("req/body/eq/obj.json")]
async fn should_fail_when_req_body_key_not_equal_to_obj() {
    post(stubr.uri()).body(json!({"notUser": {"name": "jdoe"}})).await.expect_status_not_found();
    post(stubr.uri()).body(json!({"user": {"NotName": "jdoe"}})).await.expect_status_not_found();
}

#[async_std::test]
#[stubr::mock("req/body/eq/obj.json")]
async fn should_fail_when_req_body_value_not_equal_to_obj() {
    post(stubr.uri()).body(json!({"user": {"name": "alice"}})).await.expect_status_not_found();
}

#[async_std::test]
#[stubr::mock("req/body/eq/array.json")]
async fn should_map_req_body_equal_to_array() {
    post(stubr.uri()).body(json!({"names": ["alice", "bob"]})).await.expect_status_ok();
}

#[async_std::test]
#[stubr::mock("req/body/eq/array.json")]
async fn should_fail_when_req_body_key_not_equal_to_array() {
    post(stubr.uri()).body(json!({"notNames": ["alice", "bob"]})).await.expect_status_not_found();
}

#[async_std::test]
#[stubr::mock("req/body/eq/array.json")]
async fn should_fail_when_req_body_value_not_equal_to_array() {
    post(stubr.uri()).body(json!({"names": ["a", "bob"]})).await.expect_status_not_found();
    post(stubr.uri()).body(json!({"names": ["alice", "b"]})).await.expect_status_not_found();
    post(stubr.uri()).body(json!({"names": ["alice"]})).await.expect_status_not_found();
    post(stubr.uri()).body(json!({"names": ["bob"]})).await.expect_status_not_found();
