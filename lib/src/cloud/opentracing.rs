...
impl<'a> OpenTracing<'a> {
    ...
    const OPEN_TRACING_HEADERS: [&'static str; 5] = [
        Self::TRACE_ID_KEY,
        Self::SPAN_ID_KEY,
    ...
    pub fn add_opentracing_header(
        &'a self, mut resp: ResponseTemplate, stub_headers: Option<impl Iterator<Item = &'a str>>,
    ) -> ResponseTemplate {
    ...
    fn tracing_headers(&'a self) -> impl Iterator<Item = (&'a str, &'a str)> {
        Self::OPEN_TRACING_HEADERS
            .into_iter()
            .filter_map(|k| self.req_header(k).map(|v| (k, v)))
    }

    fn req_header(&'a self, key: &'a str) -> Option<&'a str> {
        self.0.headers.get(HeaderName::from_str(key).ok().as_ref()?).map(|v| v.as_str())
    }
}
