...
mod json_body_verify_tests {
    ...
    fn should_verify_json_body() {
        ...
        resp.set_body(body);
        JsonBodyVerifier.verify(&stub, "json", &RequestData::from(&mut req), &mut StdResponse(resp));
    }

    #[should_panic(
        expected = "Verification failed for stub 'json'. Expected json response body to be '{\"name\":\"alice\"}' but was '{\"name\":\"bob\"}'"
    )]
    #[test]
    ...
    #[should_panic(
        expected = "Verification failed for stub 'json'. Expected json response body to be '{\"name\":\"alice\"}' but none present"
    )]
    #[test]
    fn verify_should_fail_when_json_body_expected_and_none_present() {
        let stub = ResponseStub {
            body: BodyStub {
                json_body: Some(json!({"name": "alice"})),
                ..Default::default()
            },
            ..Default::default()
        };
        ...
        JsonBodyVerifier.verify(&stub, "json", &RequestData::from(&mut req), &mut resp);
    }
    ...
    #[should_panic(
        expected = "Verification failed for stub 'json'. No response template transformer present but template elements present in expected response json body '{\"name\":\"{{jsonPath request.body '$.name'}}\"}'"
    )]
    #[test]
