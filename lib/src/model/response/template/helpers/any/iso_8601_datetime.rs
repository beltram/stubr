...
impl AnyIso8601Datetime {
    ...
    pub const ISO_8601_DATETIME_RGX: &'static str =
        r"([0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(\.\d+)?(Z|[+-][01]\d:[0-5]\d)";
    const REASON: &'static str = "be a valid iso 8601 datetime (yyyy-mm-ddThh:mm:ss)";
}

lazy_static! {
    pub(crate) static ref ISO_8601_DATETIME_REGEX: Regex =
        Regex::new(&format!("^{}$", AnyIso8601Datetime::ISO_8601_DATETIME_RGX)).expect("Implementation error");
}
...
impl AnyTemplate for AnyIso8601Datetime {
    ...
    fn verify<'reg: 'rc, 'rc>(
        ...
        assert!(
            ISO_8601_DATETIME_REGEX.is_match(resp),
