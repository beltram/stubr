...
pub(crate) async fn get_pooled_mock_server() -> PooledMockServer {
    MOCK_SERVER_POOL.get().await.expect("Failed to get a MockServer from the pool")
}
...
impl deadpool::managed::Manager for MockServerPoolManager {
    ...
    async fn recycle(&self, mock_server: &mut BareMockServer) -> deadpool::managed::RecycleResult<Infallible> {
