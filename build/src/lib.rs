...
impl StubrConsumer {
    ...
    fn copy_stubs(&self) {
        ...
        let output_dir = output_dir.join(Self::IMPORT_DIR).join(self.package.name()).into_path_unlocked();
        if !output_dir.exists() {
            create_dir_all(&output_dir).unwrap();
        }
        self.find_all_stubs().for_each(|(name, paths)| {
    ...
    fn find_all_stubs(&self) -> impl Iterator<Item = (String, Vec<PathBuf>)> + '_ {
        self.build_dependencies()
            .filter_map(move |d| self.src_path(d).map(|p| (d.package_name().to_string(), self.find_stubs(p))))
            .filter(|(_, files)| !files.is_empty())
    }

    fn src_path(&self, dep: &Dependency) -> Option<PathBuf> {
        dep.source_id().local_path().or_else(|| self.resolve_remote_src_path(dep))
    }
    ...
    fn build_dependencies(&self) -> impl Iterator<Item = &Dependency> {
        self.package.dependencies().iter().filter(|d| d.is_build())
    }

    fn find_stubs(&self, path: PathBuf) -> Vec<PathBuf> {
        path.join(Self::STUBS_DIR)
            .read_dir()
            .ok()
    ...
    fn resolve_package(&self) -> Resolve {
        load_pkg_lockfile(&self.workspace())
            .ok()
            .flatten()
